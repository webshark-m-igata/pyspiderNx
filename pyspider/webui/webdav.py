...
class ScriptProvider(DAVProvider):
    ...
    def get_resource_inst(self, path, environ):
        return self._get_resource_inst(path, environ)

    def _get_resource_inst(self, path, environ):
        path = path.strip('/')
        if path == '':
            return RootCollection(environ, self.app)
        else:
...
class NeedAuthController(BaseDomainController):
    def __init__(self, wsgidav_app, config):
        super(NeedAuthController, self).__init__(wsgidav_app, config)
        # Store Flask app reference
        self.app = config.get('_app', None)
        if not self.app and '_app' not in config:
    ...
    def basic_auth_user(self, realm, user_name, password, environ):
        return user_name == self.app.config.get('webui_username') \
...
config.update({
    ...
    'http_authenticator': {
        'domain_controller': NeedAuthController,
        'accept_basic': True,
        'accept_digest': False,
        'default_to_digest': False,
    },
    '_app': app,  # Pass Flask app reference
